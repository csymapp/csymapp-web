...
export default {
  ...
  methods : {
      ...
      async inactivateEmail(email, status) {
        let editedIndex = this.profiles.indexOf(email),
          editedItem = Object.assign({}, email),
          self = this,
          [err, care] = []
          email = editedItem.Email
          let emailid = editedItem.emailuid
          if(status === true)
            [err, care] = await to(authService().inactivateEmail(self.$store.state, emailid))
          else 
      ...
      async dropEmail(email) {
        ...
        if (res) {
         let editedIndex = this.profiles.indexOf(email),
          ...
          email = editedItem.Email
          let emailid = editedItem.emailuid
          ;[err, care] = await to(authService().deleteEmail(self.$store.state, emailid))
  ...
  mounted() {
    ...
    let emailProfiles = user.Emailprofiles;
