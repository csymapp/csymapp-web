      ...
      <v-tooltip bottom>
              <div slot="activator" class="layout column ma-0 justify-center align-center">
                <v-dialog v-model="basic.dialog" persistent max-width="500px">
                  ...
                  <EmaAddition @close="basic.dialog = false" color="#0077b5"></EmaAddition>
                </v-dialog>
              </div>
              <span>Add a linkedin account</span>
            </v-tooltip>
    </v-toolbar>
    </v-card-title>
        ...
        >
          <template slot="items" slot-scope="props">
            ...
            <td class="text-xs-right">
              <v-btn flat :class=whichColour(props.item.IsActive) @click="inactivateLinkedin(props.item, props.item.IsActive)">
                {{ whichText(props.item.IsActive)}}
              </v-btn>
              <v-btn flat icon color="red"  @click="dropLinkedin(props.item)">
...
import EmaAddition from '@/components/widgets/form/LinkedinAddition';
...
export default {
  ...
  methods : {
      ...
      async inactivateLinkedin(email, status) {
        let editedIndex = this.profiles.indexOf(email),
          editedItem = Object.assign({}, email),
          self = this,
          [err, care] = []
          email = editedItem.Email
          let luid = editedItem.luid
          if(status === true)
            [err, care] = await to(authService().inactivateLinkedin(self.$store.state, luid))
          else 
            [err, care] = await to(authService().activateLinkedin(self.$store.state, luid))
      ...
      async dropLinkedin(email) {
        ...
        if (res) {
         let editedIndex = this.profiles.indexOf(email),
          ...
          email = editedItem.Email
          let luid = editedItem.luid
          ;[err, care] = await to(authService().deleteLinkedin(self.$store.state, luid))
          ...
          if(err)
            ...
            let user = care.data
            self.$store.state.user.userdata = user
            window.getApp.$emit('PROFILE_REMOVED', 'Linkedin');
          }
  ...
  mounted() {
    ...
    let emailProfiles = user.Linkedins;
